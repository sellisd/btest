...
class CharacterClassifier:
    ...
    def classify_character(
        self, character: Character, script_text: Optional[str] = None
    ) -> Character:
        ...
        if script_text:
            context_gender = self._analyze_context_for_gender(
                character.name, script_text
            )
    ...
    def _get_name_gender(self, name: str) -> str:
        ...
        if first_name.endswith(("a", "ie", "y", "i")) and not first_name.endswith(
            ("by", "ey", "dy", "ty")
        ):
            return "female"
    ...
    def _analyze_context_for_gender(self, name: str, text: str) -> str:
        ...
        for sentence in sentences:
            if name in sentence:
                ...
                female_count += sum(
                    1 for word in words if word in self.female_indicators
                )
                male_count += sum(1 for word in words if word in self.male_indicators)
