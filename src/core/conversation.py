...
class ConversationAnalyzer:
            ...
            for i, (name, line) in enumerate(sequence):
                ...
                if last_speaker and name != last_speaker:
                    ...
                    if len(current_participants) >= 2:
                        logger.debug(f"Found conversation between {len(current_participants)} characters")
                ...
                if i == len(sequence) - 1 and len(current_participants) >= 2:
                    context = f"Final conversation in sequence"
                    about_men = self._analyze_topic(current_dialogue, context)
    ...
    def _analyze_topic(self, dialogue: List[str], context: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"LLM topic analysis failed for {context}, falling back to rule-based: {e}")
    ...
    def _rule_based_topic_analysis(self, dialogue: List[str]) -> bool:
        ...
        if total_words > 0:
            ...
            logger.debug(f"Rule-based analysis: {male_ref_count}/{total_words} male references = {male_ref_ratio:.2%}")
            return result
