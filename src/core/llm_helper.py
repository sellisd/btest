...
try:
    ...
    logger.warning(
        f"Could not initialize Ollama. Make sure it's running locally. Error: {e}"
    )
    llm = None
...
gender_prompt = PromptTemplate(
    input_variables=["character_name", "context"], template=GENDER_DETECTION_TEMPLATE
)

topic_prompt = PromptTemplate(
    input_variables=["dialogue"], template=CONVERSATION_TOPIC_TEMPLATE
)
...
def validate_bechdel_result(
    female_characters: List[str], conversations: List[List[str]], original_result: bool
) -> bool:
