...
def test_bechdel_score_endpoint(test_client):
    ...
    with test_client as client:
        ...
        assert response.status_code == 200, (
            f"Bechdel score request failed: {response.text}"
        )
        ...
        assert isinstance(result["female_characters"], list), (
            "female_characters should be list"
        )
        assert "num_female_conversations" in result, (
            "Result missing num_female_conversations"
        )
        assert isinstance(result["num_female_conversations"], int), (
            "num_female_conversations should be integer"
        )
...
def test_script_search_and_analyze(test_client):
    ...
    with test_client as client:
        ...
        assert isinstance(script_result["script"], str), (
            "Script content should be string"
        )
        ...
        assert hasattr(analysis, "female_characters"), (
            "Analysis missing female_characters"
        )
        assert isinstance(analysis.female_characters, list), (
            "female_characters should be list"
        )
