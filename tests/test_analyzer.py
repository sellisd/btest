...
def mock_movie_data():
    ...
    return (
        """m0 +++$+++ Test Movie +++$+++ 2020 +++$+++ 8.5 +++$+++ 1000 +++$+++ genre1"""
    )
...
def test_conversation_analyzer_finds_conversations(sample_script):
    ...
    assert all(len(conv.participants) >= 2 for conv in conversations), (
        "Each conversation should have at least 2 participants"
    )
    assert all(conv.dialogue for conv in conversations), (
        "Each conversation should have dialogue"
    )
...
def test_conversation_analyzer_finds_female_conversations(sample_script):
    ...
    assert len(female_conversations) > 0, (
        "Should find at least one conversation between women"
    )
    assert all(len(conv.participants) >= 2 for conv in female_conversations), (
        "Female conversations should have at least 2 participants"
    )
...
def test_bechdel_analyzer_full_analysis(analyzer, sample_script):
    ...
    assert len(result.female_characters) >= 2, (
        "Should identify at least 2 female characters"
    )
...
def test_analyze_movie_found(
    analyzer, mock_movie_data, mock_lines_data, mock_conversations_data
):
    """Test analyzing movie by title when script is found."""
    with patch(
        "builtins.open", mock_open(read_data=mock_movie_data)
    ) as mock_movies_file, patch(
        "builtins.open", mock_open(read_data=mock_lines_data)
    ) as mock_lines_file, patch(
        "builtins.open", mock_open(read_data=mock_conversations_data)
    ) as mock_conv_file, patch("requests.get") as mock_get:
...
def test_analyze_movie_not_found(analyzer, mock_movie_data, mock_lines_data):
    ...
    with patch(
        ...
        "builtins.open", mock_open(read_data=mock_lines_data)
    ) as mock_lines_file, patch("requests.get") as mock_get:
        mock_get.return_value.text = "mock data"
